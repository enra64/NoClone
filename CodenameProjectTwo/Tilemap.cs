...
namespace CodenameProjectTwo
{
    class TileMap
    {
                ...
                for (uint x = 0; x < tileAmount.X; x++)
                {
                    ...
                    switch (tileType)
                    {
                        case CGlobal.EARTH_TILE:
                            ...
                            break;
                        case CGlobal.LAVA_TOP_TILE:
                            if (y < tileAmount.Y)
                                Collidable[x, y] = true;
                            xOffset = LAVA1TOPOFFSET;
                            break;
                        case CGlobal.LAVATILE:
                            if (y < tileAmount.Y)
                                Collidable[x, y] = true;
                            xOffset = LAVA1OFFSET;
                            break;
                        case CGlobal.EARTH_TOP_TILE:
                            xOffset = EARTHTOPTILEOFFSET;
                            if (y < tileAmount.Y)
                                Collidable[x, y] = true;
                            break;
                        case CGlobal.SPAWNTILE_1:
                            xOffset = AIROFFSET;
                            spawnPoints.Add(new int[] { (int)x, (int)y, CGlobal.SPAWNTILE_1 });
                            if (y < tileAmount.Y)
                                Collidable[x, y] = false;
                            break;
                        case CGlobal.SPAWNTILE_2:
                            xOffset = AIROFFSET;
                            spawnPoints.Add(new int[] { (int)x, (int)y, CGlobal.SPAWNTILE_2 });
                            if (y < tileAmount.Y)
                                Collidable[x, y] = false;
                            break;
                        case CGlobal.SPAWNTILE_3:
                            xOffset = AIROFFSET;
                            spawnPoints.Add(new int[] { (int)x, (int)y, CGlobal.SPAWNTILE_3 });
                            if (y < tileAmount.Y)
                                Collidable[x, y] = false;
                            break;
                        default:
        ...
        /// <summary>
        /// Returns the tile type defined in CGlobal (see GetCurrentTile for
        ...
        private void fillTileTypeArray(string levelLocation)
        {
            ...
            for (int y = 0; y < tileAmount.Y; y++)
            {
                for (int x = 0; x < tileAmount.X; x++)
                {
                    ...
                    //default to air tile
                    tileTypes[x, y] = CGlobal.AIR_TILE;
        ...
        public void AnimationUpdate()
        {
            for (uint y = 0; y < tileAmount.Y + yInterpolationDuration; y++)
            {
                for (uint x = 0; x < tileAmount.X; x++)
                {
                    ...
                    else if (tileType == CGlobal.LAVA_TOP_TILE)
                    {
                        //+0 should contain pure x offset
                        texCo1 = tileMap[currentPosition + 0].TexCoords;
                        Vector2f newOffset = new Vector2f(0, 0);
